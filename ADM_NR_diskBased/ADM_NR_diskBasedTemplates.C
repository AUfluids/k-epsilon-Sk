...
void Foam::fv::ADM_NR_diskBased::calc
(
...
template<class AlphaFieldType, class RhoFieldType>
void Foam::fv::ADM_NR_diskBased::calcFroudeMethod
(
    const AlphaFieldType& alpha,
    const RhoFieldType& rho,
    fvMatrix<vector>& eqn
)
{
    ...
    const scalar T = 2.0*rhoRef*diskArea_*magSqr(Uref & diskDir_)*(a/(1.0 - a)); 
...
template<class AlphaFieldType, class RhoFieldType>
void Foam::fv::ADM_NR_diskBased::calcVariableScalingMethod
(
    const AlphaFieldType& alpha,
    const RhoFieldType& rho,
    fvMatrix<vector>& eqn
)
{
    ...
    // Calibrated thrust/power coeffs from power/thrust curves 
    const scalar a = 1.0 - Cp/Ct; 				
